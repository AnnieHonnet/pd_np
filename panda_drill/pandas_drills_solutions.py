#!/usr/bin/env python3
"""
pandas_drills_solutions.py -- worked answers for pandas_drills.py

Run:  python pandas_drills_solutions.py          all exercises
      python pandas_drills_solutions.py 14       just exercise 14

Every exercise here passes its own asserts. Consult it when stuck, but try
the blank version first -- typing the answer is where the learning happens.
"""
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = pd.read_csv(DATA_DIR / "coverage_summary.tsv", sep="\t",
                         na_values=["-"])
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = [c for c in d.raw.columns
              if not pd.api.types.is_numeric_dtype(d.raw[c])]
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.loc[(d.cov["rname"] == "Cdiff_CDSM_1")
                       & (d.cov["coverage"] > 98)]
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    wanted = ["Mouse1_Day1", "Mouse2_Day3", "Mouse3_Day4"]
    result = d.cov.loc[d.cov["sample"].isin(wanted)]
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    length_kb = (result["endpos"] - result["startpos"] + 1) / 1000
    result["reads_per_kb"] = result["numreads"] / length_kb
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["rname"] = result["rname"].replace({
        "Cdiff_CDSM_1": "chromosome",
        "Cdiff_CDSM_2": "plasmid",
    })
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["low_depth"] = False
    result.loc[result["meandepth"] < 15, "low_depth"] = True
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["depth_class"] = pd.cut(
        result["meandepth"], bins=[0, 20, 40, 80],
        labels=["low", "mid", "high"],
    )
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result.loc[result["rname"] == "Cdiff_CDSM_2", "meanmapq"] = 0
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    extra = pd.DataFrame([
        {"sample": "Mouse4_Day1", "rname": "Cdiff_CDSM_1", "startpos": 1,
         "endpos": 4152362, "numreads": 41000, "covbases": 4000000,
         "coverage": 98.0, "meandepth": 41, "meanbaseq": 36.0,
         "meanmapq": 55.0, "mouse": 4, "day": 1},
        {"sample": "Mouse4_Day1", "rname": "Cdiff_CDSM_2", "startpos": 1,
         "endpos": 8000, "numreads": 2050, "covbases": 7800,
         "coverage": 98.0, "meandepth": 82, "meanbaseq": 36.0,
         "meanmapq": 50.0, "mouse": 4, "day": 1},
    ])
    result = pd.concat([d.cov, extra], ignore_index=True)
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.rename(columns={"rname": "contig", "numreads": "n_reads"})
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.drop(columns=["meanbaseq", "meanmapq", "covbases"])
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    order = ["sample", "mouse", "day", "rname", "meandepth", "coverage"]
    result = d.cov[order]
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.raw.copy()
    result["sample"] = result["sample"].str.strip()
    parts = result["sample"].str.extract(r"Mouse(\d+)_Day(\d+)")
    result["mouse"] = parts[0].astype(int)
    result["day"] = parts[1].astype(int)
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.loc[d.cov["sample"].str.contains("Day3")]
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.mpa.copy()
    result["species"] = (result["clade_name"]
                         .str.split("|").str[-1]
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["label"] = ("M" + result["mouse"].astype(str)
                       + "D" + result["day"].astype(str))
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    indexed = d.cov.set_index(["sample", "rname"])
    result = indexed.loc[("Mouse2_Day3", "Cdiff_CDSM_1"), "meandepth"]
    # <<< SOLUTION
    assert np.isscalar(result) or isinstance(result, (int, np.integer)), \
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.sort_values(["mouse", "meandepth"],
...
def ex(d):
    # >>> SOLUTION
    result = d.cov.dropna(subset=["coverage"])
    # <<< SOLUTION
    assert len(result) == 34, f"expected 34 rows, got {len(result)}"
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    fill = result.groupby("rname")["coverage"].transform("mean")
    result["coverage"] = result["coverage"].fillna(fill)
    # <<< SOLUTION
...
def ex(d):
    # >>> SOLUTION
    result = d.cov.groupby("mouse")["meandepth"].mean()
    # <<< SOLUTION
    assert isinstance(result, pd.Series), "expected a Series"
...
def ex(d):
    # >>> SOLUTION
    result = d.cov.groupby(["mouse", "day"])["numreads"].sum()
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.groupby("mouse").agg(
        mean_depth=("meandepth", "mean"),
        max_depth=("meandepth", "max"),
        total_reads=("numreads", "sum"),
        n=("sample", "size"),
    )
    # <<< SOLUTION
    assert list(result.columns) == \
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["mouse_mean_depth"] = (result.groupby("mouse")["meandepth"]
                                        .transform("mean"))
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.groupby("mouse").filter(lambda g: g["meandepth"].mean() > 30)
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.groupby("rname").size()
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = (d.k2.pivot_table(index="sample", columns="name", values="reads")
                  .reset_index())
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    chrom = d.cov.loc[d.cov["rname"] == "Cdiff_CDSM_1"]
    cdiff = d.k2.loc[d.k2["taxid"] == 1496, ["sample", "reads"]]
    result = chrom.merge(cdiff, on="sample", how="left")
    # <<< SOLUTION
...
def ex(d):
    ...
    chrom = d.cov.loc[d.cov["rname"] == "Cdiff_CDSM_1"]
    cdiff = d.k2.loc[d.k2["taxid"] == 1496, ["sample", "reads"]]
    result = chrom.merge(cdiff, on="sample", how="inner")
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    chrom = d.cov.loc[d.cov["rname"] == "Cdiff_CDSM_1"]
    result = chrom.merge(lookup, left_on="sample", right_on="sample_id",
                         how="left")
    # <<< SOLUTION
    assert len(result) == 18
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    merged = d.cov.merge(d.cov, on="sample", how="inner")
    result = len(merged)
    # <<< SOLUTION
    assert result == 72, \
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.mpa.melt(id_vars="clade_name", var_name="sample",
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    long = d.mpa.melt(id_vars="clade_name", var_name="sample",
                      value_name="abundance")
    result = long.pivot(index="sample", columns="clade_name",
                        values="abundance")
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    result = d.cov.pivot_table(index="mouse", columns="rname",
                               values="numreads", aggfunc="sum")
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    mapping = {1: "pre", 2: "pre", 3: "peak", 4: "peak", 5: "post", 6: "post"}
    result = d.cov.copy()
    result["phase"] = result["day"].map(mapping)
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["depth_log10"] = result["meandepth"].apply(np.log10)
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["tag"] = result.apply(
        lambda row: f"{row['sample']}:{row['rname']}", axis=1
    )
    # <<< SOLUTION
...
def ex(d):
    ...
    # >>> SOLUTION
    result = d.cov.copy()
    result["molecule"] = np.where(result["endpos"] < 100000,
                                  "plasmid", "chromosome")
    # <<< SOLUTION
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    k2_cdiff = (d.k2.loc[d.k2["taxid"] == 1496, ["sample", "reads"]])

    mpa_long = d.mpa.melt(id_vars="clade_name", var_name="sample",
                          value_name="abundance")
    mpa_cdiff = mpa_long.loc[
        mpa_long["clade_name"].str.contains("Clostridioides_difficile"),
        ["sample", "abundance"],
    ]

    result = mpa_cdiff.merge(k2_cdiff, on="sample", how="left")
    result = result[["sample", "reads", "abundance"]]
    # <<< SOLUTION
    assert len(result) == 18, f"expected 18 rows, got {len(result)}"
...
def ex(d):
    ...
    merged = mpa_cdiff.merge(k2_cdiff, on="sample", how="inner")
    merged["reads_per_abundance"] = merged["reads"] / merged["abundance"]
    result = (merged.sort_values("reads_per_abundance", ascending=False)
                    .head(3)[["sample", "reads_per_abundance"]]
...
def ex(d):
    ...
    """
    # >>> SOLUTION
    summary = (d.cov.loc[d.cov["rname"] == "Cdiff_CDSM_1",
                         ["sample", "mouse", "day", "numreads",
                          "meandepth", "coverage"]]
                    .sort_values(["mouse", "day"])
                    .reset_index(drop=True))
    out = DATA_DIR / "summary.tsv"
